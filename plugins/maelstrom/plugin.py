...
class MaelstromPlugin(PluginBase):
    ...
    def __init__(self, config: AASConfig, hub: Any):
        ...
        self._latest_home_snapshot: Optional[Dict[str, Any]] = None
    ...
    async def setup(self) -> bool:
        ...
        self._state = (
            MaelstromState.STOPPED if not self._process else MaelstromState.RUNNING
        )
    ...
    def _discover_installation(self) -> None:
        ...
        if not self._maelstrom_root:
            workspace_path = Path(__file__).parent.parent.parent / "Maelstrom"
    ...
    def _find_executable(self) -> None:
        ...
        logger.warning(
            "Maelstrom executable not found. May need to build or set MAELSTROM_START_CMD."
        )
        ...
        for plugin_path in plugins_dir.iterdir():
            ...
            if manifest_path.exists():
                try:
                    ...
                    logger.warning(
                        f"Failed to load sub-plugin manifest {manifest_path}: {e}"
                    )
            else:
    ...
    def _register_ipc_handlers(self) -> None:
        ...
        handlers = {
            ...
            "home.gateway.update": self.handle_home_gateway_update,
            "home.gateway.status": self.handle_home_gateway_status,
        }

        for cmd, handler in handlers.items():
            if hasattr(self.hub, "register_ipc_handler"):
                self.hub.register_ipc_handler(cmd, handler)
    ...
    async def start_maelstrom(self) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.broadcast_event(
                "maelstrom_started",
                {"pid": self._process.pid, "executable": str(self._executable)},
            )
    ...
    async def handle_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "home_gateway": self._latest_home_snapshot,
        }
    ...
    async def handle_home_gateway_update(
        self, snapshot: Optional[Dict[str, Any]] = None, **kwargs
    ) -> Dict[str, Any]:
        """
        Accept status updates from the Home Gateway (Hub-side plugin).

        This keeps Maelstrom aware of environmental/home telemetry without coupling
        game automation to the gateway implementation.
        """
        self._latest_home_snapshot = snapshot or kwargs
        logger.debug("Maelstrom received Home Gateway snapshot update")
        return {"success": True}

    async def handle_home_gateway_status(self) -> Dict[str, Any]:
        """Return the last Home Gateway snapshot received via IPC."""
        if not self._latest_home_snapshot:
            return {"success": False, "error": "No home gateway snapshot received"}
        return {"success": True, "home_gateway": self._latest_home_snapshot}
    ...
    async def handle_execute_command(
        self, command_type: str, payload: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        if self._state != MaelstromState.RUNNING:
            return {
                "success": False,
                "error": f"Maelstrom not running (state: {self._state.value})",
            }
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        return {
            **super().get_info(),
            "state": self._state.value,
            "maelstrom_root": (
                str(self._maelstrom_root) if self._maelstrom_root else None
            ),
