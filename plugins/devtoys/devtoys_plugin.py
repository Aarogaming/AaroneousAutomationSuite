...
class DevToysPlugin:
    ...
    def _validate_sdk_path(self):
        ...
        if not self.config.sdk_path:
            logger.warning("DevToys SDK path not provided; skipping validation")
            return
    ...
    async def perform_task(self, data: str) -> dict:
        """Compatibility shim for legacy API used by tests."""
    ...
    async def start(self):
        ...
        # Create task list
        tasks = [
            self.run_task(f"Task-{i}") for i in range(self.config.max_concurrent_tasks)
        ]
        ...
        logger.info(
            f"DevToys startup complete: {successes}/{len(tasks)} tasks successful"
        )
    ...
    async def execute_batch(self, task_names: List[str]) -> dict:
        ...
        return {name: result for name, result in zip(task_names, results)}
