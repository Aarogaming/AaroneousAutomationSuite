...
class GameAutomationPlugin(PluginBase):
    ...
    async def handle_follow_route(
        self, route_name: str, forward: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        await self.broadcast_event(
            "route_started", {"route": route_name, "forward": forward}
        )
        ...
        for i, point in enumerate(points):
            ...
            if not result.get("success"):
                await self.broadcast_event(
                    "route_error", {"route": route_name, "point": i}
                )
                return result
    ...
    async def handle_trainer_start(
        self, name: str, difficulty: str = "easy", **kwargs
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if result.get("success"):
                ...
                await self.broadcast_event(
                    "trainer_started", {"name": name, "difficulty": difficulty}
                )
    ...
    async def handle_trainer_stop(self, name: str) -> Dict[str, Any]:
        ...
        if result.get("success"):
            await self.broadcast_event(
                "trainer_stopped",
                {"name": name, "final_score": result.get("final_score", 0)},
            )
