        ...
        try:
            while self._state == TrainerState.RUNNING:
                ...
                if game_state.get("game_over"):
                    ...
                    if self.config.auto_retry and self._attempts < self.config.max_retries:
                ...
                for arrow in arrows:
                    if await self._should_press(arrow):
                        success = await self._execute_action("press", direction=arrow["direction"])
                        if success:
                            self._score += 1
        ...
        if action == "press":
            ...
            if self.adapter:
                result = await self.adapter.send_key(
                    key, 
                    self.config.hold_duration_ms / 1000.0
                )
