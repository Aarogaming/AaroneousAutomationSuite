...
Tests GuildManager's claim_next_task, complete_task, and dependency logic.
...
"""
        ...
        cls.manager = GuildManager(artifact_dir=cls.artifact_dir)
    ...
    def test_priority_based_claiming(self):
        ...
        self.assert_equals(
            claimed.get("id"), "TEST-001", "Claimed highest priority task (Urgent)"
        )
        ...
        self.assert_equals(
            test001["status"], "In Progress", "Task status updated to In Progress"
        )
    ...
    def test_dependency_blocking(self):
        ...
        self.assert_true(claimed is not None, "Task was claimed")
        self.assert_equals(
            claimed.get("id"), "TEST-002", "Skipped blocked task, claimed next eligible"
        )
    ...
    def test_complete_task_unblocking(self):
        ...
        self.assert_true(claimed is not None, "Task was claimed")
        self.assert_equals(
            claimed.get("id"), "TEST-003", "Previously blocked task now claimable"
        )
    ...
    def test_multiple_dependencies(self):
        ...
        self.assert_equals(
            claimed.get("id"),
            "TEST-008",
            "Correctly skipped task with incomplete multi-deps",
        )
        ...
        self.assert_equals(
            claimed.get("id"),
            "TEST-004",
            "Task with multiple deps claimable after all complete",
        )
    ...
    def test_blocked_tasks_list(self):
        ...
        self.assert_true(
            "TEST-007" in blocked_ids, "TEST-007 is blocked by missing dep"
        )
    ...
    def test_artifact_scaffolding(self):
        ...
        if claimed:
            ...
            self.assert_true(
                ...
                f"Artifact directory created: {artifact_path}",
            )
            self.assert_true(
                os.path.exists(readme_path), "README.md created in artifact directory"
            )
            ...
            with open(readme_path, "r", encoding="utf-8") as f:
                ...
                self.assert_true(
                    "ArtifactTester" in content, "README contains assignee"
                )
...
"""
        ...
        self.assert_equals(
            claimed1.get("id"), "FAIR-001", "First task claimed among same priority"
        )

        # Second claim should get FAIR-002
        claimed2 = self.manager.claim_next_task("Actor2")
        self.assert_equals(
            claimed2.get("id"), "FAIR-002", "Second task claimed in order"
        )

        # Third claim should get FAIR-003
        claimed3 = self.manager.claim_next_task("Actor3")
        self.assert_equals(
            claimed3.get("id"), "FAIR-003", "Third task claimed in order"
        )
    ...
    def test_parse_board_status_map(self):
        ...
        self.assert_equals(
            status_map.get("TEST-001"), "queued", "Queued task in status map"
        )
        self.assert_equals(
            status_map.get("TEST-005"), "Done", "Done task in status map"
        )
        self.assert_equals(
            status_map.get("TEST-006"), "In Progress", "In Progress task in status map"
        )
...
"""
        ...
        self.assert_equals(
            claimed.get("id"), "CIRC-003", "Claims non-circular task instead"
        )

        blocked = self.manager.get_blocked_tasks()
        blocked_ids = [t["id"] for t in blocked]
        self.assert_true(
            "CIRC-001" in blocked_ids and "CIRC-002" in blocked_ids,
