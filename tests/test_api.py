...
def test_api_connection():
    ...
    try:
        ...
        logger.info(
            f"  Available models: {models.data[0].id if models.data else 'None'}"
        )
        ...
        logger.success("✓ Completion test successful!")
...
def test_responses_api():
    ...
    try:
        ...
        logger.success("✓ Responses API test successful!")
        logger.info(f"  Response ID: {response.id}")
        logger.info(
            f"  Message: {response.output[0].content[0].text if response.output else 'No output'}"
        )
