"""
GuildManager integration with database layer.

Extends GuildManager to persist tasks, events, and artifacts to the database.
"""
...
from core.db_models import TaskStatus, TaskPriority, EventType
...
class DatabaseGuildMixin:
    """
    Mixin to add database persistence to GuildManager.

    Usage:
        class GuildManager(DatabaseGuildMixin):
            def __init__(self, ...):
                super().__init__(...)
                self.init_database()
    """
    ...
    def start_task_execution(self, task_id: str, agent: str) -> int:
        """
        ...
        with self.db.get_session() as session:
            execution = TaskExecutionRepository.create(
                session, task_id=task_id, agent=agent
            )
            ...
            return [
                {
                    ...
                    "started_at": (
                        ex.started_at.isoformat() if ex.started_at is not None else None
                    ),
                    "completed_at": (
                        ex.completed_at.isoformat()
                        if ex.completed_at is not None
                        else None
                    ),
