...
class TaskExecutionRepository:
    ...
    @staticmethod
    def create(
        session: Session, task_id: str, agent: str, status: str = "in_progress"
    ) -> TaskExecution:
        """Create a new task execution."""
        execution = TaskExecution(task_id=task_id, agent=agent, status=status)
...
class PluginRepository:
    ...
    def get_all_enabled(session: Session) -> List[Plugin]:
        ...
        return session.query(Plugin).filter(Plugin.status == PluginStatus.ENABLED).all()

    @staticmethod
    def update_status(
        session: Session, name: str, status: PluginStatus, error: Optional[str] = None
    ) -> Optional[Plugin]:
...
class ConfigRepository:
    ...
    def _get_fernet(cls) -> Fernet:
        ...
        if cls._fernet is None:
            ...
            if not key:
                logger.warning(
                    "AAS_ENCRYPTION_KEY not found in environment. Generating a temporary one."
                )
...
class KnowledgeRepository:
    ...
    def create_edge(
        ...
        logger.info(
            f"Created knowledge edge: {source_id} --[{relationship_type}]--> {target_id}"
        )
        return edge
...
class GuildRepository:
    """Repository for Guild model operations."""
    ...
    def create(
        ...
        guild_id = f"guild-{uuid.uuid4().hex[:12]}"
        ...
        logger.info(f"Created guild record: {guild_id} for {task_id}")
        return guild

    @staticmethod
    def get_by_task(session: Session, task_id: str) -> List[Guild]:
        """Get all guilds for a task."""
        return (
            session.query(Guild)
            .filter(Guild.task_id == task_id)
