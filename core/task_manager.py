"""
Unified Task Manager - Combines Guild, AutoBatch, and Task Tracking

This module unifies:
- GuildManager: Task claiming, status updates, health monitoring
...
class TaskManager(ManagerProtocol):
    """
    Unified Task Manager combining Guild, AutoBatch, and Task Tracking.
    ...
    def __init__(
        ...
        if not os.getenv("OPENAI_API_KEY"):
            ...
            if not os.getenv("OPENAI_API_KEY"):
                ...
                logger.debug(
                    "OPENAI_API_KEY not found; using placeholder for initialization"
                )
        ...
        self.workspace_coordinator = workspace or WorkspaceCoordinator(
            workspace_root="."
        )
        self.artifact_manager = artifacts or ArtifactManager(
            base_dir=self.config.artifact_dir
        )

        # Initialize GuildManager if not provided
        if not guild:
            try:
                # Try to find GuildManager in various locations
                try:
                ...
                if GuildManager:
                    self.guild = GuildManager(config=self.config)
                else:
                    self.guild = None
            except Exception as e:
                logger.warning(f"Failed to initialize GuildManager: {e}")
                self.guild = None
        else:
            self.guild = guild
        ...
        try:
            ...
            logger.warning(
                f"Could not create task board directory {self.board_path.parent}: {e}"
            )
        if not self.board_path.exists():
            # Try legacy locations for backward compatibility
            potential_paths = [
                Path(os.getcwd()) / "artifacts" / "guild" / "ACTIVE_TASKS.md",
                Path(os.getcwd()) / "guild" / "ACTIVE_TASKS.md",
            ]
        ...
        logger.info(
            "TaskManager initialized with unified interface + workspace coordination"
        )
    ...
    def find_unbatched_tasks(self, max_count: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        if not self.guild:
            logger.warning("Guild manager not initialized")
            return []

        lines, tasks, status_map = self.guild.parse_board()
    ...
    def find_next_claimable_task(
        self, exclude_batched: bool = True
    ...
    async def _broadcast_task_event(
        self, task_id: str, title: str, status: str, assignee: str, event_type: str
    ):
    ...
    def claim_task(
        self, task_id: Optional[str] = None, actor_name: str = "GitHub Copilot"
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        with self.db.get_session() as session:
            if task_id:
                ...
                if status_val != TaskStatus.QUEUED.value:
                    logger.warning(
                        f"Task {task_id} is not queued (status: {status_val})"
                    )
                    return None
            else:
                ...
                eligible = []
                status_map = {
                    t.id: t.status for t in session.query(Task.id, Task.status).all()
                }
            ...
            # Broadcast update (best-effort if an event loop is running)
            try:
                ...
                logger.debug("No running event loop; skipping broadcast_task_event")
    ...
    def _sync_task_to_markdown(self, task: Task):
        ...
        if not self.guild:
            return
        try:
            lines, tasks, _ = self.guild.parse_board()
            ...
            if target:
                ...
                with open(self.guild.task_board_path, "w", encoding="utf-8") as f:
                    f.writelines(lines)
        except Exception as e:
    ...
    def get_task_status(self, task_id: str) -> Dict[str, Any]:
        ...
        """
        if not self.guild:
            return {"error": "Guild manager not initialized"}
    ...
    async def decompose_and_add_tasks(
        self, goal: str, priority: str = "medium", task_type: str = "feature"
    ):
        """
        Decompose a high-level goal into sub-tasks and add them to the system.
        """
        from core.agents import TaskDecomposer
        ...
        with self.db.get_session() as session:
            for dep in dependencies:
                ...
                if task_title in title_to_id and depends_on_title in title_to_id:
                    ...
                    if task:
                        current_deps = (
                            list(task.dependencies) if task.dependencies else []
                        )
    ...
    def add_task(
        ...
        with self.db.get_session() as session:
            ...
            try:
                ...
                # No running loop (e.g., CLI sync context); skip async broadcast
                logger.debug("No running event loop; skipping broadcast_task_event")
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        for t in tasks:
            ...
            if t["status"] == "queued" and (
                t["priority"].lower() in ["urgent", "high"]
            ):
                if not t["assignee"] or t["assignee"] == "-":
                    health["unassigned_high_priority"].append(
                        {"id": task_id, "title": t["title"], "priority": t["priority"]}
                    )
    ...
    def generate_health_report(self) -> str:
        ...
        report_path = self.artifact_manager.store_report(
            "HEALTH_REPORT.md", "\n".join(report)
        )
    ...
    def get_all_tasks(self) -> Dict[str, Any]:
        ...
        _, tasks, status_map = self.guild.parse_board()
        ...
        return {
            ...
            "status_counts": {
                ...
                "in_progress": sum(
                    1 for t in tasks if t["status"].lower() == "in progress"
                ),
    ...
    def register_client(
        self, client_id: str, hostname: str, client_type: str = "worker"
    ) -> bool:
        ...
        with self.db.get_session() as session:
            ...
            if not client:
                client = Client(
                    id=client_id, hostname=hostname, client_type=client_type
                )
                session.add(client)
            else:
        ...
        with self.db.get_session() as session:
            ...
            if client:
                ...
                if cpu_usage is not None:
                    client.cpu_usage = cpu_usage  # type: ignore
                if mem_usage is not None:
                    client.mem_usage = mem_usage  # type: ignore
                return True
            return False
    ...
    def check_client_timeouts(self, timeout_seconds: int = 90):
        ...
        with self.db.get_session() as session:
            ...
            stale_clients = (
                ...
                .filter(Client.last_heartbeat < cutoff, Client.status == "online")
                .all()
            )
