    ...
    async def _analyze_goal(self, state: DecompositionState) -> Dict[str, Any]:
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a senior technical architect. Analyze the following goal and identify key technical components and potential challenges.",
                ),
                ("user", "{goal}"),
            ]
        )
    ...
    async def _generate_subtasks(self, state: DecompositionState) -> Dict[str, Any]:
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """Break down the goal into 3-7 actionable sub-tasks.
...
Format as a JSON list of objects.""",
        ...
        return {"subtasks": subtasks, "messages": [response]}
    ...
    async def _map_dependencies(self, state: DecompositionState) -> Dict[str, Any]:
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """Given the following list of sub-tasks, identify dependencies between them.
