...
class GameModeManager:
    ...
    async def enter(
        ...
        if self._state in (GameModeState.ENTERING, GameModeState.EXITING):
            return {
                "success": False,
                "error": f"Transition in progress: {self._state.value}",
            }
        ...
        timeout = (
            ...
            else self._config.idle_timeout_minutes
        )
        do_start_maelstrom = (
            start_maelstrom
            if start_maelstrom is not None
            else self._config.start_maelstrom
        )
        do_enable_trainers = (
            enable_trainers
            if enable_trainers is not None
            else self._config.enable_trainers
        )
    ...
    async def exit(self, reason: str = "manual") -> Dict[str, Any]:
        ...
        try:
            ...
            if self._game_automation_plugin:
                ...
                try:
                    ...
                    for trainer_name in status.get("trainers", {}).keys():
                        await self._game_automation_plugin.handle_trainer_stop(
                            trainer_name
                        )
                except Exception as e:
            ...
            # Broadcast event
            await self._broadcast_event(
                "game_mode_exited", {"reason": reason, "session": session_summary}
            )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        if self._session:
            result["session"] = {
                ...
                "automation_idle_minutes": round(
                    self._session.automation_idle_minutes, 2
                ),
                "commands_executed": self._session.commands_executed,
                "snapshots_received": self._session.snapshots_received,
    ...
    async def _broadcast_event(self, event_type: str, data: Dict[str, Any]) -> None:
        ...
        elif hasattr(self._hub, "ws_manager") and hasattr(
            self._hub.ws_manager, "broadcast"
        ):
            await self._hub.ws_manager.broadcast({"type": event_type, "data": data})
