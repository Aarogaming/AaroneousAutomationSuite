...
from core.db_models import AgentSession, HelpRequest, TaskLock
from core.protocol_manager import GuildObject
...
class AgentCollaborationManager:
    ...
    def check_in(
        ...
        caps = capabilities or self.capabilities.get(
            agent_name,
            {
                "strengths": ["general"],
                "languages": ["python"],
    ...
    def check_out(self, session_id: str):
        ...
        with self.db.get_session() as session:
            ...
            if agent_session is not None:
                agent_session.status = "offline"  # type: ignore
                agent_session.last_activity = datetime.utcnow()  # type: ignore
                session.commit()
    ...
    def _print_agent_roster(self):
        ...
        for agent in agents:
            ...
            task = (
                f" [Working: {agent['current_task']}]" if agent["current_task"] else ""
            )
    ...
    def acquire_task_lock(
        ...
        with self.db.get_session() as session:
            ...
            existing_lock = session.query(TaskLock).filter_by(task_id=task_id).first()

            if existing_lock is not None and existing_lock.session_id != session_id:
                # Check if lock is expired
                if (
                    existing_lock.expires_at is not None
                ...
                elif str(existing_lock.lock_type) in ["active", "soft"]:
                    # Task already locked by another agent
                    lock_owner = (
                        session.query(AgentSession)
                        .filter_by(id=existing_lock.session_id)
                        .first()
                    )
            ...
            if agent_session is not None:
                if lock_type == "active":
                    agent_session.current_task_id = task_id  # type: ignore
                    agent_session.active_tasks_count = (agent_session.active_tasks_count or 0) + 1  # type: ignore

                session.commit()
                logger.info(
                    f"🔒 {agent_session.agent_name} acquired {lock_type} lock on {task_id}"
                )
            return True
    ...
    def release_task_lock(self, task_id: str, session_id: str):
        ...
        with self.db.get_session() as session:
            lock = (
                ...
                .filter_by(task_id=task_id, session_id=session_id)
                .first()
            )
            ...
            if lock is not None:
                ...
                # Update agent session
                agent_session = (
                    session.query(AgentSession).filter_by(id=session_id).first()
                )
                if lock_type == "active" and agent_session is not None:
                    agent_session.current_task_id = None  # type: ignore
    ...
    def request_help(
        ...
        with self.db.get_session() as session:
            # Get requester info
            requester = (
                session.query(AgentSession).filter_by(id=requester_session_id).first()
            )
            ...
            if requester is not None:
                requester.help_requests_count = (requester.help_requests_count or 0) + 1  # type: ignore

            session.commit()

            if requester:
                logger.info(
                    f"🆘 {requester.agent_name} requesting {help_type} help on {task_id}"
                )
    ...
    def accept_help_request(
        ...
        with self.db.get_session() as session:
            ...
            if help_request is None or str(help_request.status) != "open":
                return False

            # Update help request
            help_request.helper_session_id = helper_session_id  # type: ignore
            help_request.status = "accepted"  # type: ignore
            help_request.accepted_at = datetime.utcnow()  # type: ignore
            help_request.response_message = response_message  # type: ignore
            ...
            requester = (
                session.query(AgentSession)
                .filter_by(id=help_request.requester_session_id)
                .first()
            )

            if helper and requester:
                logger.success(
                    f"🤝 {helper.agent_name} accepted help request from {requester.agent_name}"
                )
            return True
    ...
    def complete_help_request(self, request_id: str, outcome: str):
        ...
        with self.db.get_session() as session:
            ...
            if help_request is not None:
                help_request.status = "completed"  # type: ignore
                help_request.completed_at = datetime.utcnow()  # type: ignore
                help_request.response_message = outcome  # type: ignore

                # Release helper lock
                if help_request.helper_session_id is not None:
                    lock = (
                        session.query(TaskLock)
                        .filter_by(
                            task_id=help_request.task_id,
    ...
    def _broadcast_help_request(self, request_id: str):
        ...
        with self.db.get_session() as session:
            ...
            for agent in active_agents:
                if agent["session_id"] == str(help_req.requester_session_id):
                    continue  # Skip requester

                caps = agent["capabilities"]
                match_score = self._calculate_capability_match(
                    str(help_req.help_type), caps
                )
                if match_score > 0.5:
                    print(
                        f"    ✓ {agent['agent_name']} (match: {int(match_score*100)}%)"
                    )
            print()
    ...
    def find_best_agent_for_task(
        self, task_description: str, task_tags: Optional[List[str]] = None
        ...
        for agent in active_agents:
            ...
            score = self._calculate_task_match(
                task_description, task_tags or [], agent["capabilities"]
            )
            ...
            scored_agents.append({**agent, "match_score": final_score})
    ...
    def _calculate_capability_match(
        self, help_type: str, capabilities: Dict[str, Any]
    ) -> float:
    ...
    def _calculate_task_match(
        self, task_description: str, task_tags: List[str], capabilities: Dict[str, Any]
    ) -> float:
        ...
        is_private_task = (
            "private" in task_description.lower() or "private" in task_tags
        )
        if (
            is_private_task
            and self.config
            and not getattr(self.config, "is_private_version", False)
        ):
    ...
    def relay_guild(self, guild: GuildObject):
        """
        Relay guild context to the target agent or store it for the next claimant.
        Implementation for AAS-212.
        """
        logger.info(
            f"Relaying guild from {guild.source_agent} for task {guild.task_id}"
        )

        from core.db_repositories import GuildRepository

        with self.db.get_session() as session:
            GuildRepository.create(
                session,
                task_id=guild.task_id,
                source_agent=guild.source_agent,
                target_agent=guild.target_agent,
                context_summary=guild.context_summary,
                technical_details=guild.technical_details,
                relevant_files=guild.relevant_files,
                pending_actions=guild.pending_actions,
            )
        logger.success(
            f"Guild context for {guild.task_id} persisted via CollaborationManager."
        )
