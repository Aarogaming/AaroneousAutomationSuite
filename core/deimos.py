...
class DeimosVM:
    ...
    def __init__(self, clients: List[Any]):
        self.clients: List[Any] = clients
        self.running: bool = False
        self.killed: bool = False
        self.instructions: List[str] = []
        self.pc: int = 0  # Program Counter

    def load_from_text(self, script_text: str) -> None:
        """Parses and loads a DeimosLang script into memory."""
        logger.info("Loading DeimosLang script...")
        self.instructions = script_text.splitlines()
        self.pc = 0

    async def step(self) -> None:
        """Execute a single instruction if available."""
    ...
    async def run(self) -> None:
        """Run the loaded script until completion or stop signal."""
        self.running = True
