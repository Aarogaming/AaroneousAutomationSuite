...
class WorkspaceCoordinator:
    ...
    def find_duplicates(
        self, scan_dirs: Optional[List[str]] = None
    ) -> Dict[str, List[Path]]:
        """
        ...
        logger.info(
            f"Scanned {file_count} files, found {len(duplicates)} duplicate sets"
        )
        return duplicates
    ...
    def detect_runaway_bot(self, time_window_minutes: int = 5) -> Dict[str, Any]:
        ...
        for paths in duplicates.values():
            recent_in_set = sum(
                1 for p in paths if any(p == rf[0] for rf in recent_files)
            )
            if recent_in_set > 1:
        ...
        is_runaway = (
            excessive_creation
            or recent_duplicate_count > self.max_duplicates_threshold
            or max_files_in_dir > 100
        )
        ...
        result = {
            ...
            "threshold_exceeded": {
                ...
                "too_many_duplicates": recent_duplicate_count
                > self.max_duplicates_threshold,
        ...
        if is_runaway:
            logger.warning(
                f"Runaway bot detected! Recent files: {len(recent_files)}, "
                f"Rate: {files_per_minute:.1f}/min"
            )
        else:
            logger.info(
                f"No runaway activity detected. Recent files: {len(recent_files)}"
            )

        return result

    def cleanup_duplicates(
        self, dry_run: bool = True, keep_strategy: str = "newest"
    ) -> List[Path]:
        """
    ...
    def defrag_workspace(self, dry_run: bool = True) -> List[str]:
        ...
        results = []
        logger.info(
            f"Starting workspace defragmentation {'(dry run)' if dry_run else ''}..."
        )

        # 1. Consolidate Maelstrom build outputs
        publish_dir = self.workspace_root / "Maelstrom/src/ProjectMaelstrom/publish"
        ...
        ui_dirs = [
            "Maelstrom/src/ProjectMaelstrom/ui_audit_pack_selfcapture",
            "Maelstrom/src/ProjectMaelstrom/ui_baseline",
            "Maelstrom/src/ProjectMaelstrom/ui_current",
        ]
        for d in ui_dirs:
        ...
        doc_source = self.workspace_root / "Maelstrom"
    ...
    def generate_workspace_report(self) -> Dict[str, Any]:
        ...
        temp_waste_mb = sum(
            f.stat().st_size / (1024 * 1024) for f in temp_files if f.exists()
        )
    ...
    def _calculate_health_score(
        self, duplicate_mb: float, temp_mb: float, runaway: Dict, git_compat: Dict
    ) -> str:
    ...
    def save_report(
        self, report: Dict, output_path: str = "artifacts/workspace_health.json"
    ):
