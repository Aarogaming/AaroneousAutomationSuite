...
from core.db_models import TaskPriority, TaskStatus
...
class BackgroundWorker:
    ...
    """
    def __init__(self, task_manager, max_workers: int = 4):
    ...
    async def start(self):
        ...
        for priority in TaskPriority:
            self.workers[priority] = [
                ...
                for _ in range(self.max_workers if priority == TaskPriority.URGENT else 2)
            ]
    ...
    async def enqueue(self, task_id: str, priority: TaskPriority = TaskPriority.MEDIUM):
        """Enqueue a task for background processing."""
        await self.queues[priority].put(task_id)
        logger.debug(f"Enqueued task {task_id} with priority {priority}")

    async def process_queue(self, priority: TaskPriority):
