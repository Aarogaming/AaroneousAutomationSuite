...
class BatchAutoClear:
    ...
    def archive_old_results(self) -> int:
        ...
        cutoff_date = datetime.utcnow() - timedelta(days=self.criteria["archive_age_days"])
        archived_count = 0
    ...
    def evaluate_criteria(self, state: Dict[str, Any]) -> Dict[str, bool]:
        ...
        results["max_active_batches"] = len(active_batches) <= self.criteria["max_active_batches"]
        
        # Criterion 2: Completion rate
        total = len(active_batches) + len(completed_batches)
        if total > 0:
            completion_rate = len(completed_batches) / total
            results["completion_rate"] = completion_rate >= self.criteria["min_completion_rate"]
        else:
            results["completion_rate"] = True  # No batches = no problem
        
        # Criterion 3: Old pending batches
        old_batches = []
        cutoff_time = datetime.utcnow() - timedelta(hours=self.criteria["max_pending_age_hours"])
        
        for batch_id, batch_info in active_batches.items():
            submitted_at = datetime.fromisoformat(batch_info.get("submitted_at", datetime.utcnow().isoformat()))
    ...
    def auto_clear_backlog(self, dry_run: bool = False) -> Dict[str, Any]:
        ...
        for batch_id, batch_info in list(active_batches.items()):
            ...
            if batch_status["status"] == "completed":
                ...
                logger.warning(f"Batch {batch_id} has terminal status: {batch_status['status']}")
                stats["batches_failed"] += 1
                if not dry_run:
        ...
        if not dry_run and newly_completed:
            ...
            state["stats"]["tasks_processed"] = state["stats"].get("tasks_processed", 0) + len(newly_completed)
            
            self.save_monitor_state(state)
            logger.success(f"Updated monitor state with {len(newly_completed)} newly completed batch(es)")
