...
class BatchMonitor:
    ...
    def __init__(
        ...
        self.guild = GuildManager(config)
        self.batcher = AutoBatcher(
            config, dry_run=dry_run, max_tasks=max_tasks_per_batch
        )
        self.processor = BatchProcessor(config) if not dry_run else None
        self.linear = self.guild.linear
        self.team_id = getattr(config, "linear_team_id", None)
        self.task_generator = TaskGenerator(self.guild, self.linear, config)
        ...
        if self.state_file.exists():
            try:
                with open(self.state_file, "r") as f:
                    ...
                    logger.info(
                        f"Loaded state: {len(self.submitted_tasks)} submitted tasks, "
    ...
    def _get_board_hash(self) -> str:
        ...
        if not os.path.exists(self.guild.task_board_path):
            return ""
        with open(self.guild.task_board_path, "r", encoding="utf-8") as f:
            content = f.read()
    ...
    async def scan_for_eligible_tasks(self) -> List[Dict[str, Any]]:
        ...
        new_eligible = [
            task for task in all_eligible if task["id"] not in self.submitted_tasks
        ]

        if new_eligible:
            logger.info(
                f"Found {len(new_eligible)} new eligible tasks (filtered {len(all_eligible) - len(new_eligible)} already submitted)"
            )
            for task in new_eligible:
                logger.success(f"  ✅ NEW: {task['id']} ({task['title']})")
        else:
            logger.debug(
                f"No new eligible tasks (all {len(all_eligible)} already submitted)"
            )
    ...
    async def submit_batch(
        ...
        if len(self.active_batches) >= self.max_concurrent_batches:
            logger.warning(
                f"Already at max concurrent batches ({self.max_concurrent_batches}), skipping submission"
            )
            return None
        ...
        try:
            ...
            if result:
                ...
                logger.info(
                    f"Active batches: {len(self.active_batches)}/{self.max_concurrent_batches}"
                )
    ...
    async def _update_linear_batched(self, task_ids: List[str], batch_id: str):
        ...
        try:
            ...
            if not target_state:
                logger.warning(
                    "Could not find 'Batched' or 'In Progress' workflow state"
                )
                return
...
Results will be automatically posted when processing completes."""
            ...
            for task_id in task_ids:
                try:
                    ...
                    if not issue_id:
                        logger.warning(
                            f"Could not get/create Linear issue for {task_id}, skipping"
                        )
                        continue
        ...
        for batch_id, metadata in list(self.active_batches.items()):
            try:
                ...
                if new_status == "completed":
                    ...
                    logger.info(
                        f"Tasks from batch now available for re-submission: {', '.join(task_ids)}"
                    )
    ...
    async def _process_batch_results(self, batch_id: str, metadata: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(
                f"Extracted {len(task_responses)} task responses from batch {batch_id}"
            )
            ...
            # Update Linear tasks
            if self.linear and self.team_id:
                await self._update_linear_completed(
                    metadata["task_ids"], batch_id, task_responses, results_file
                )

            # Update statistics
            self.stats["tasks_processed"] += len(task_responses)
            estimated_cost_saved = (
                len(task_responses) * 2.50
            )  # ~$2.50 per task @ 50% savings
    ...
    def _get_or_create_linear_issue(self, task_id: str) -> Optional[str]:
        ...
        # Issue doesn't exist - create it from guild board
        logger.info(f"Creating Linear issue for {task_id} (doesn't exist yet)")

        try:
            task_details = self.batcher.get_task_details(task_id)
            if not task_details:
                logger.error(f"Could not find task {task_id} in guild board")
                return None

            description = f"Auto-created from AAS guild board for batch processing.\n\n**Priority:** {task_details.get('priority', 'medium')}\n**Status:** {task_details.get('status', 'queued')}"
            ...
            issue_id = self.linear.create_issue(
                self.team_id, task_details["title"], description
            )
    ...
    async def _update_linear_completed(
        self,
        task_ids: List[str],
        batch_id: str,
...
Please review the recommendations and mark as Done if satisfactory."""
    ...
    async def run_task_generation_cycle(self):
        ...
        try:
            ...
            if total_tasks >= 500:
                logger.warning(
                    f"🛑 Task creation SUSPENDED: {total_tasks}/500 tasks on board"
                )
                ...
                logger.info(
                    f"📊 Task board status: {total_tasks} tasks (normal operation)"
                )
            else:
            ...
            logger.info(
                f"  → {len(progress_suggestions)} suggestions from progress review"
            )
            ...
            logger.info(
                f"  → {len(improvement_suggestions)} suggestions from improvement analysis"
            )
            ...
            if all_suggestions:
                high_priority = [
                    s for s in all_suggestions if s.get("priority") == "high"
                ]
                other_priority = [
                    s for s in all_suggestions if s.get("priority") != "high"
                ]
                ...
                if high_priority:
                    ...
                    created = await self.task_generator.create_tasks(
                        high_priority, dry_run=self.dry_run
                    )
                ...
                if other_priority:
                    ...
                    for suggestion in other_priority:
                        logger.info(
                            f"  • [{suggestion.get('priority')}] {suggestion.get('title')}"
                        )

            logger.info("=" * 60)
            logger.info("Next task generation cycle in 6 hours")
            logger.info("=" * 60)
    ...
    def print_status(self):
        ...
        if self.active_batches:
            ...
            for batch_id, meta in self.active_batches.items():
                logger.info(
                    f"  {batch_id}: {meta['status']} ({len(meta['task_ids'])} tasks)"
                )
        ...
        try:
            while True:
                ...
                if task_generation_task is None or task_generation_task.done():
                    task_generation_task = asyncio.create_task(
                        self.run_task_generation_cycle()
                    )
...
async def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Test mode (no actual submissions)"
    )
    parser.add_argument(
        "--interval",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1800,
        help="Batch check interval in seconds (default: 1800 = 30 min)",
    )
    parser.add_argument(
        "--max-batches", type=int, default=5, help="Max concurrent batches (default: 5)"
    )
    parser.add_argument(
        "--max-tasks", type=int, default=20, help="Max tasks per batch (default: 20)"
    )
