...
class BatchRecycler:
    ...
    def extract_actionable_items(self, content: str, original_task_id: str) -> List[Dict[str, str]]:
        """
        ...
        for match in matches:
            ...
            cleaned_details = '\n'.join([line.strip() for line in details_lines if line.strip()])
        ...
        if not items:
            generic_step_pattern = r'(?:Step |(?:^\d+\.\s+)|(?:Phase \d+:))\s*([^\n]+?)(?:\n|$)'
    ...
    def recycle_batch(self, result_file: Path, dry_run: bool = False) -> int:
        ...
        for item in items:
            title = item["title"]
            description = f"**Source:** Batch result from {task_id}\n\n{item['description']}"
    ...
    def recycle_all_pending(self, dry_run: bool = False, force: bool = False) -> Dict[str, Any]:
        """
        ...
        for result_file in result_files:
            ...
            try:
                ...
                if not dry_run and batch_id not in already_processed:
                    ...
                    log_data["total_tasks_created"] = log_data.get("total_tasks_created", 0) + tasks_created
...
def main():
    ...
    parser.add_argument("--force", action="store_true", help="Reprocess already-processed batches")
