...
def demo_collaboration():
    ...
    copilot_session = collab.check_in("GitHub Copilot", agent_version="1.0")

    chatgpt_session = collab.check_in("ChatGPT", agent_version="4.0")

    sixth_session = collab.check_in("Sixth", agent_version="1.0")
    ...
    best_agent = collab.find_best_agent_for_task(
        task_desc, task_tags=["refactoring", "testing", "documentation"]
    )
    ...
    print("💡 In production, Sixth would acquire a lock on a real task")
    print("   This prevents other agents from working on the same task")
    print("   Lock types: 'active' (full control), 'soft' (intent), 'helper' (read)")
    ...
    print("   In production, this would be a real task ID from the database\n")

    # Skip actual help request to avoid FK constraint
    print("🆘 Sixth would request: code_review")
    print("   Context: Need review on new TaskManager refactoring before commit")
    print("   Urgency: high, Estimated: 15 minutes\n")
    ...
    print("💡 In production, Copilot would see the help request and accept it")
    print("   This creates a 'helper' lock (non-exclusive) so multiple agents")
    print("   can assist without blocking the primary owner\n")
    ...
    print(
        "   'Reviewed code. Suggested minor improvements to error handling. LGTM overall!'\n"
    )
