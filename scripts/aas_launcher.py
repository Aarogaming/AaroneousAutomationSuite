...
def find_listener_pids(ports: Iterable[int]) -> List[int]:
    ...
    for conn in psutil.net_connections(kind="inet"):
        if conn.laddr and conn.status == psutil.CONN_LISTEN and conn.laddr.port in ports:
            if conn.pid:
                pids.add(conn.pid)
    return sorted(pids)
...
def pick_python_executable() -> Path:
    ...
    if sys.platform.startswith("win"):
        ...
        candidates = [
            ...
            Path(sys.executable),
            PROJECT_ROOT / ".venv" / "Scripts" / "python.exe",  # last resort on WSL if bin missing
        ]
...
def run_preflight(web_host: str = "0.0.0.0", web_port: int = 8000):
    ...
    config = load_config()
    result = run_preflight(config=config, workspace_root=PROJECT_ROOT, web_host=web_host, web_port=web_port)
    result.log()
    if result.issues:
...
def do_launch(args: argparse.Namespace) -> None:
    ...
    if args.wait:
        ...
        for attempt in range(1, attempts + 1):
            web_ready = wait_for_port(web_port, host="127.0.0.1", timeout=args.wait_timeout)
            ipc_ready = wait_for_port(ipc_port, host="127.0.0.1", timeout=args.wait_timeout)
        ...
        if web_ready and ipc_ready:
            ...
            logger.error(f"Readiness timed out after {attempts} attempts (web_ready={web_ready}, ipc_ready={ipc_ready}); check artifacts/hub.log")
            raise SystemExit(1)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="AAS Launcher (prep + preflight + hub start)")
    sub = parser.add_subparsers(dest="command", required=True)

    launch = sub.add_parser("launch", help="Prep workspace, run preflight, and start the Hub")
    launch.add_argument("--no-clean-temp", action="store_true", help="Skip temp file cleanup")
    launch.add_argument("--no-kill-zombies", action="store_true", help="Skip killing listeners on 50051/8000")
    launch.add_argument("--inline-tray", action="store_true", help="Start tray inline if available")
    launch.add_argument("--wait", action="store_true", help="Wait for web server to come up")
    launch.add_argument("--wait-timeout", type=float, default=15.0, help="Seconds to wait when --wait is set")
    launch.add_argument("--wait-retries", type=int, default=3, help="Number of readiness retry attempts")
    launch.add_argument("--wait-interval", type=float, default=2.0, help="Seconds between readiness retries")
    launch.add_argument("--log-max-mb", type=int, default=100, help="Rotate hub.log above this size")
    launch.add_argument("--web-port", type=int, default=8000, help="Port to probe when waiting for readiness")
